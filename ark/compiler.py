...
class ArkCompiler():
    ...
    def verify(self, cdg: CDG, cdg_spec: CDGSpec):
        ...
        for node in cdg.nodes:
            candidate_rules = cdg_spec.get_candidate_val_rules(node=node)
            print(node)
    ...
    def generate(self, cdg: CDG, cdg_spec: CDGSpec):
        ...
        for node in cdg.stateful_nodes:
            for edge in node.edges:
                rhs = cdg_spec.apply_gen_rule(edge=edge, src=edge.src, dst=edge.dst, tgt=node.is_dst(edge))
...
if __name__ == '__main__':
    ...
    # Generation Rules
    from ark.specification.generation_rule import GenRule, RewriteGen
    gen_rules = []
