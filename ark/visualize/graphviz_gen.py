...
for r, g, b in palettable.cartocolors.qualitative.Bold_10.colors:
    if too_light(r, g, b):
        continue
    GraphVizFormatter.add_node_style(
        NodeStyle(color=(r, g, b, 255), shading=NodeShading.SOLID)
    )
...
for idx, (r, g, b) in enumerate(palettable.cartocolors.qualitative.Prism_10.colors):
    ...
    GraphVizFormatter.add_edge_style(
        EdgeStyle(color=(r, g, b, 255), shading=options[idx % len(options)])
    )
...
for idx, (r, g, b) in enumerate(palettable.cmocean.sequential.Gray_10.colors):
    ...
    GraphVizFormatter.add_inherited_node_style(
        NodeStyle(color=(val, val, val, 255), shading=NodeShading.SOLID)
    )
    GraphVizFormatter.add_inherited_edge_style(
        EdgeStyle(color=(val, val, val, 255), shading=options[idx % len(options)])
    )
...
class RenderableGraph:
    ...
    def load_cdg_lang(self, lang):
        ...
        for idx, n in enumerate(lang.node_types()):
            if idx >= len(GraphVizFormatter.INHERITED_NODE_STYLES) or idx >= len(
                GraphVizFormatter.NODE_STYLES
            ):
        ...
        for idx, n in enumerate(lang.edge_types()):
            if idx >= len(GraphVizFormatter.INHERITED_EDGE_STYLES) or idx >= len(
                GraphVizFormatter.EDGE_STYLES
            ):
    ...
    def save(self, subdir):
        ...
        if (
            "ratio" not in self.graph.graph_attr
            or self.graph.graph_attr["ratio"] is None
        ):
