...
class LatexPrettyPrinter:
    ...
    def math_formula(cls, expr):
        ...
        if (
            LatexPrettyPrinter.ESCAPE_STYLE == EscapeStyle.NONE
            or LatexPrettyPrinter.ESCAPE_STYLE == EscapeStyle.TABULAR
            or LatexPrettyPrinter.ESCAPE_STYLE == EscapeStyle.VERBATIM
        ):
...
class LatexVerbatim:
    ...
    def linebreak(self, pad=""):
        lb = LatexVerbatim.LineBreak(
            indent_style=self.indent_style, n_indent=self.n_indent, pad=pad, force=False
        )
        self.ctx.append(lb)

    def newline(self):
        lb = LatexVerbatim.LineBreak(
            indent_style=self.indent_style, n_indent=self.n_indent, pad="", force=True
        )
        self.ctx.append(lb)
