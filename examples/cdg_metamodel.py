...
TargType = NodeType(name=subscr("NT", "i"), order=1, reduction=SUM, attr_def=[])

n_types_rev = [subscr("NT", "1"), subscr("NT", "j"), subscr("NT", "m")]
NodeTypesRev = {}
EdgeTypesRev = {}
for name in n_types_rev:
    NodeTypesRev[name] = NodeType(name=name, order=1, reduction=SUM, attr_def=[])

e_types_rev = [subscr("ET", "1"), subscr("ET", "j"), subscr("ET", "m")]
for name in e_types_rev:
    EdgeTypesRev[name] = EdgeType(name=name, attr_def=[])
...
EdgeTypes = {}
for name in n_types:
    NodeTypes[name] = NodeType(name=name, order=1, reduction=SUM, attr_def=[])

e_types = [subscr("ET", "m+1"), subscr("ET", "k"), subscr("ET", "n")]
for name in e_types:
    EdgeTypes[name] = EdgeType(name=name, attr_def=[])
