...
SwE = EdgeType(name='SwE',attr_def=[AttrDef('ctrl', attr_type=FunctionType),
...
def sinosoidal(t):
    ...
    return np.sin(2 * np.pi * 1e2 * t)

# Production rules
switch_cap_conn = ProdRule(SwE, InpV, Cap, DST, (SRC.fn(TIME) - VAR(DST)) / SRC.r / 
                           DST.c * EDGE.ctrl(TIME, EDGE.period, EDGE.duty_cycle, EDGE.offset))
...
for i in range(N_PATH):
    caps[i] = Cap(c=1e-2)
    switches[i] = SwE(ctrl=ctrl_clk, offset=PERIOD / N_PATH * i, period=PERIOD, duty_cycle=DUTY_CYCLE)
...
sol = compiler.prog(TIME_RANGE, init_states=init_states, init_seed=SEED, max_step=PERIOD/100)
time_points = sol.t
...
for i in range(0, N_PATH):
    ...
    output += sol.y[mapping[caps[i]]] * np.array([ctrl_clk(t, PERIOD, DUTY_CYCLE, PERIOD / N_PATH * i)
