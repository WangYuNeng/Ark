...
Coupling = EdgeType(name='Cpl', attr_def=[AttrDef('k', attr_type=float,
                                                  attr_range=Range(min=-8, max=8))])

Osc_group = [NodeType(name=f'Osc_G{i}', base=Osc) for i in range(N_GROUP)]
# local connection that has a lower cost
Coupling_local = EdgeType(name='Cpl_l', base=Coupling, attr_def=[AttrDef('cost',
                                                                         attr_type=int,
                                                                         attr_range=Range(exact=1))])
# global connection that has a higher cost
Coupling_global = EdgeType(name='Cpl_g', base=Coupling, attr_def=[AttrDef('cost',
                                                                          attr_type=int,
                                                                          attr_range=Range(exact=10))])
...
for i in range(N_GROUP):
    ...
    val_rules.append(ValRule(node_type, [ValPattern(SELF, Coupling_local,
                                                    node_type, Range(exact=1)),
                                         ValPattern(SRC, Coupling_local,
                                                    node_type, Range(min=0)),
                                         ValPattern(DST, Coupling_local,
                                                    node_type, Range(min=0)),
                                         ValPattern(SRC, Coupling_global,
                                                    Osc, Range(min=0)),
                                         ValPattern(DST, Coupling_global,
                                                    Osc, Range(min=0))]
                            ))
