...
if __name__ == "__main__":
    ...
    n_path_filter = CDG()
    inp_vs = [
        InpV(fn=partial(sinosoidal, freq=INP_FREQ)),
        InpV(fn=zero),
    ]
    inp_ress = [Res(r=1.0), Res(r=1.0)]
    n_path_filter.connect(Wire(), inp_vs[0], inp_ress[0])
    n_path_filter.connect(Wire(), inp_vs[1], inp_ress[1])
    ...
    for i in range(N_PATH):
        ...
        n_path_filter.connect(SwE(ctrl=ctrl_clk), inp_ress[0], caps[i])

        n_path_filter.connect(
            SwE(ctrl=ctrl_clk), inp_ress[1], caps[(i + N_PATH // 2) % N_PATH]
        )
    ...
    ax[-1].plot(time_points, output[0] - output[1], label="Output")
    ax[-1].legend()
    plt.tight_layout()
    plt.show()
